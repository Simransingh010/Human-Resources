    ...
    <flux:table :paginate="$this->list" class="w-full">
        ...
        <flux:table.rows>
            @foreach ($this->list as $app)
                <flux:table.row :key="$app->id" class="border-b">
                    <flux:table.cell class="table-cell-wrap">
                        <div class="flex items-center gap-2">
                            @if($app->color)
                                <div class="w-3 h-3 rounded-full" style="background-color: {{ $app->color }}"></div>
                            @endif
                            {{ $app->name }}
                        </div>
                    </flux:table.cell>
                    <flux:table.cell class="table-cell-wrap">{{ $app->code }}</flux:table.cell>
                    <flux:table.cell>
                        @if($app->icon)
                            <flux:icon :name="$app->icon" class="w-5 h-5"/>
                        @endif
                    </flux:table.cell>
                    <flux:table.cell class="table-cell-wrap">{{ $app->route }}</flux:table.cell>
                    <flux:table.cell>{{ $app->order }}</flux:table.cell>
                    <flux:table.cell>
                        <flux:switch
                            wire:model="statuses.{{ $app->id }}"
                    ...
                    <flux:table.cell>
                        <div class="flex space-x-2">
                            ...
                            <flux:modal.trigger name="delete-{{ $app->id }}">
                                <flux:button variant="danger" size="sm" icon="trash"/>
                            </flux:modal.trigger>
                        </div>

                        <!-- Delete Modal -->
                        <flux:modal name="delete-{{ $app->id }}" class="min-w-[22rem]">
                            <div class="space-y-6">
                                <div>
