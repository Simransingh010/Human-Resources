    ...
    <flux:table :paginate="$this->list" class="">
        ...
        <flux:table.rows>
            @foreach ($this->list as $panel)
                <flux:table.row :key="$panel->id" class="border-b">
                    <flux:table.cell class="table-cell-wrap">{{ $panel->name }}</flux:table.cell>
                    <flux:table.cell class="table-cell-wrap">{{ $panel->code }}</flux:table.cell>
                    <flux:table.cell class="table-cell-wrap">{{ $panel->description }}</flux:table.cell>
                    <flux:table.cell class="table-cell-wrap">{{ ucfirst($panel->panel_type_label) }}</flux:table.cell>
                    <flux:table.cell>
                        <flux:switch
                                wire:model="statuses.{{ $panel->id }}"
                                wire:click="toggleStatus({{ $panel->id }})"
                        />
                    </flux:table.cell>
                    <!-- In your actions cell -->
                    <flux:table.cell>
                        <div class="flex space-x-2">
                            <flux:button wire:click="showAppSync({{ $panel->id }})" color="zinc" size="xs">App
                            </flux:button>
                            <flux:button wire:click="showModuleSync({{ $panel->id }})" color="zinc" size="xs">Module
                            </flux:button>
                            <flux:button
                                    variant="primary"
                                    size="sm"
                                    icon="pencil"
                                    wire:click="edit({{ $panel->id }})"
                            />

                            <flux:modal.trigger name="delete-{{ $panel->id }}">
