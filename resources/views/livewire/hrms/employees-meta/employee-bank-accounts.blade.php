<div>
    <div class="flex justify-between">
        <div>
            <flux:heading size="lg" class="flex">
                ...
                Bank Account ({{$this->employee->fname}} {{$this->employee->lname}} | {{$this->employee->email}})
            </flux:heading>
            <flux:subheading>
    ...
    <flux:modal name="mdl-bank-account" @close="resetForm" position="right" class="max-w-none">
        <flux:heading size="lg">{{$this->employee->fname}} {{$this->employee->lname}} | {{$this->employee->email}} </flux:heading>
        ...
        <form wire:submit.prevent="saveBankAccount">
            <div class="space-y-6">
                <div>
                    <flux:heading size="lg" class="flex">
                        <flux:icon.building-library />
                        @if($isEditing) Edit Bank Account @else Bank Account @endif
                    </flux:heading>
                    <flux:subheading>
                        Configure employee bank account details.
                    </flux:subheading>
