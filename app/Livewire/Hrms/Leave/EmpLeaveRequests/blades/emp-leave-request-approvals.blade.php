<div class="space-y-6">
    ...
    <div class="flex justify-between">
        ...
        <flux:modal.trigger name="mdl-leave-request-approval" class="flex justify-end">
    ...
    <!-- Add/Edit Leave Request Approval Modal -->
    <flux:modal name="mdl-leave-request-approval" @cancel="resetForm">
        <form wire:submit.prevent="store">
            <div class="space-y-6">
                <div>
                    <flux:heading size="lg">
                        @if($isEditing) Edit Leave Request Approval @else Add Leave Request Approval @endif
                    </flux:heading>
                    <flux:subheading>
                        @if($isEditing) Update @else Add new @endif leave request approval details.
                    </flux:subheading>
                </div>
                ...
                <flux:table.row :key="$item->id">
                            ...
                            <flux:table.cell class="table-cell-wrap">
                                @switch($field)
                                    ...
                                    @case('approver_id')
                                        ...
                                        @break
                                    @case('acted_at')
                                        {{ $item->acted_at ? $item->acted_at->format('Y-m-d H:i:s') : 'N/A' }}
                                        @break
                                    @case('status')
                                        {{ $listsForFields['statuses'][$item->status] ?? 'N/A' }}
                                        @break
                                    @case('created_at')
                    ...
                    <flux:table.cell>
                        ...
                        <flux:modal name="delete-{{ $item->id }}" class="min-w-[22rem]">
                            <div class="space-y-6">
                                <div>
                                    <flux:heading size="lg">Delete Leave Request Approval?</flux:heading>
                                    <flux:text class="mt-2">
                                        <p>You're about to delete this leave request approval. This action cannot be undone.</p>
                                    </flux:text>
                                </div>
