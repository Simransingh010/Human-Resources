<div class="space-y-6">
    ...
    <!-- Modal Start -->
    <flux:modal name="mdl-calendar" @cancel="resetForm" position="right" class="max-w-none" variant="flyout">
        <form wire:submit.prevent="store">
            <div class="space-y-6">
                <div>
                    ...
                    <flux:subheading>
                        @if($isEditing) Update @else Add new @endif holiday calendar details.
                    </flux:subheading>
                </div>
    ...
    <flux:modal name="calendar-holidays-modal" wire:model="showHolidaysModal" title="Calendar Holidays" class="max-w-6xl">
        @if($selectedCalendarId)
            <livewire:hrms.work-shifts.holidays 
                :holiday-calendar-id="$selectedCalendarId"
                :wire:key="'calendar-holidays-'.$selectedCalendarId"/>
        @endif
    </flux:modal>
    ...
    <flux:table :paginate="$this->list">
        <flux:table.columns class="bg-zinc-200 dark:bg-zinc-800 border-b dark:border-zinc-700">
            @foreach($fieldConfig as $field => $cfg)
                ...
                @endif
            @endforeach
            <flux:table.column>Holidays Count</flux:table.column>
            ...
            @foreach ($this->list as $rec)
                <flux:table.row :key="$rec->id" class="border-b">
                    @foreach($fieldConfig as $field => $cfg)
                        ...
                        @endif
                    @endforeach
                    <flux:table.cell class="table-cell-wrap">{{ $rec->holidays->count() }}</flux:table.cell>
                    <flux:table.cell>
                        <div class="flex space-x-2">
                            <flux:button
                                wire:click="showCalendarHolidays({{ $rec->id }})"
                                color="blue"
                                size="sm"
                            >
                                Holidays
                        ...
                        <flux:modal name="delete-{{ $rec->id }}" class="min-w-[22rem]">
                            <div class="space-y-6">
                                <div>
                                    <flux:heading size="lg">Delete Holiday Calendar?</flux:heading>
                                    <flux:text class="mt-2">
                                        <p>You're about to delete this holiday calendar. This action cannot be undone.</p>
                                        <p class="mt-2 text-red-500">Note: Calendars with holidays cannot be deleted.</p>
                                    </flux:text>
                                </div>
