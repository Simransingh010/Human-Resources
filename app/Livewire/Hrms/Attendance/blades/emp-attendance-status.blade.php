<div class="space-y-6">
    ...
    <div class="flex justify-between">
        ...
        <flux:modal.trigger name="mdl-attendance-status" class="flex justify-end">
    ...
    <!-- Add/Edit Attendance Status Modal -->
    <flux:modal name="mdl-attendance-status" @cancel="resetForm">
        <form wire:submit.prevent="store">
            <div class="space-y-6">
                <div>
                    <flux:heading size="lg">
                        @if($isEditing) Edit Attendance Status @else Add Attendance Status @endif
                    </flux:heading>
                    <flux:subheading>
                        @if($isEditing) Update @else Add new @endif attendance status details.
                    </flux:subheading>
                </div>
                ...
                <flux:table.row :key="$item->id">
                            ...
                            <flux:table.cell>
                                @switch($cfg['type'])
                                    @case('switch')
                                        ...
                                        @break
                                    @case('select')
                                        @if($field === 'attendance_status_main')
                                            {{ $listsForFields['attendance_status_main'][$item->$field] ?? $item->$field }}
                                        @elseif($field === 'work_shift_id')
                                            {{ $listsForFields['work_shifts'][$item->$field] ?? 'N/A' }}
                                        @else
                                            {{ $item->$field }}
                                        @endif
                                        @break
                    ...
                    <flux:table.cell>
                        ...
                        <flux:modal name="delete-{{ $item->id }}" class="min-w-[22rem]">
                            <div class="space-y-6">
                                <div>
                                    <flux:heading size="lg">Delete Attendance Status?</flux:heading>
                                    <flux:text class="mt-2">
                                        <p>You're about to delete this attendance status. This action cannot be undone.</p>
                                        <p class="mt-2 text-red-500">Note: Attendance statuses with related records cannot be deleted.</p>
                                    </flux:text>
                                </div>
