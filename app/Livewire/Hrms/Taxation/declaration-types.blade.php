<div class="space-y-6">
    <div class="flex justify-between">
        ...
        <flux:modal.trigger name="mdl-declaration-type" class="flex justify-end">
    ...
    <flux:modal name="mdl-declaration-type" @cancel="@this.resetForm()">
        <form wire:submit.prevent="store">
            <div class="space-y-6">
                <div>
                    <flux:heading size="lg">Add Declaration Type</flux:heading>
                    <flux:subheading>Define a new declaration type and limits.</flux:subheading>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <flux:select label="Declaration Group (optional)" wire:model.live="formData.declaration_group_id">
                        <option value="">Select Group</option>
                        @foreach(($lists['declaration_groups'] ?? []) as $id => $label)
                            <option value="{{ $id }}">{{ $label }}</option>
                        @endforeach
                    </flux:select>
                    <div class="flex items-center gap-3">
                        <flux:switch label="Proof Required" wire:model.live="formData.proof_required" />
                    </div>
                    <div class="md:col-span-2">
                        <flux:textarea rows="3" label="Validation Rules (JSON or text)" wire:model.live="formData.validation_rules" />
                    </div>
                </div>
    ...
    <div class="relative">
        ...
        <flux:table>
            ...
            <flux:table.rows>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="text-center py-8 text-gray-400 text-lg">No declaration types found.</td>
                    </tr>
                @endforelse
            </flux:table.rows>
        </flux:table>
