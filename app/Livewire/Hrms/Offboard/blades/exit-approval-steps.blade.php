<div class="space-y-6">
    ...
    <div class="flex justify-between">
        ...
        <flux:modal.trigger name="mdl-exit-approval-step" class="flex justify-end">
    ...
    <!-- Add/Edit Exit Approval Step Modal -->
    <flux:modal name="mdl-exit-approval-step" @cancel="resetForm">
        <form wire:submit.prevent="store">
            <div class="space-y-6">
                <div>
                    <flux:heading size="lg">
                        @if($isEditing) Edit Exit Approval Step @else Add Exit Approval Step @endif
                    </flux:heading>
                    <flux:subheading>
                        @if($isEditing) Update @else Add new @endif exit approval step details.
                    </flux:subheading>
                </div>
                            ...
                            @switch($cfg['type'])
                                @case('select')
                                    <flux:select
                                        ...
                                        required="{{ in_array($field, ['exit_employee_department_id', 'exit_employee_designation_id', 'flow_order', 'approval_type', 'department_id']) }}"
                                    >
                                ...
                                @case('number')
                                    <flux:input
                                        ...
                                        wire:model.live="formData.{{ $field }}"
                                        min="1"
                                        required="{{ in_array($field, ['flow_order']) }}"
                                    />
                                    @error("formData.{$field}")
                                        <flux:text color="red" size="sm">{{ $message }}</flux:text>
                                    @enderror
                ...
                <flux:table.row :key="$item->id">
                            ...
                            <flux:table.cell>
                                @switch($cfg['type'])
                                    @case('switch')
                                        ...
                                        @break
                                    @case('select')
                                        @if($field === 'exit_employee_department_id')
                                            {{ $item->exitEmployeeDepartment->title ?? 'N/A' }}
                                        @elseif($field === 'exit_employee_designation_id')
                                            {{ $item->exitEmployeeDesignation->title ?? 'N/A' }}
                                        @elseif($field === 'department_id')
                                            {{ $item->department->title ?? 'N/A' }}
                                        @elseif($field === 'approval_type')
                                            {{ $item->approval_type_label ?? $item->$field }}
                                        @elseif($field === 'is_inactive')
                                            {{ $listsForFields['inactive_options'][$item->$field] ?? $item->$field }}
                                        @else
                                            {{ $item->$field }}
                                        @endif
                                        @break
                    ...
                    <flux:table.cell>
                        ...
                        <flux:modal name="delete-{{ $item->id }}" class="min-w-[22rem]">
                            <div class="space-y-6">
                                <div>
                                    <flux:heading size="lg">Delete Exit Approval Step?</flux:heading>
                                    <flux:text class="mt-2">
                                        <p>You're about to delete this exit approval step. This action cannot be undone.</p>
                                        <p class="mt-2 text-red-500">Note: Exit approval steps with related approval actions cannot be deleted.</p>
                                    </flux:text>
                                </div>
