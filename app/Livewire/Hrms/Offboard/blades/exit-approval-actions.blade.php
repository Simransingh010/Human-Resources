<div class="space-y-6">
    ...
    <div class="flex justify-between">
        ...
        <flux:modal.trigger name="mdl-exit-approval-action" class="flex justify-end">
    ...
    <!-- Add/Edit Exit Approval Action Modal -->
    <flux:modal name="mdl-exit-approval-action" @cancel="resetForm">
        <form wire:submit.prevent="store">
            <div class="space-y-6">
                <div>
                    <flux:heading size="lg">
                        @if($isEditing) Edit Exit Approval Action @else Add Exit Approval Action @endif
                    </flux:heading>
                    <flux:subheading>
                        @if($isEditing) Update @else Add new @endif exit approval action details.
                    </flux:subheading>
                </div>
                            ...
                            @switch($cfg['type'])
                                @case('select')
                                    <flux:select
                                        ...
                                        required="{{ in_array($field, ['exit_approval_step_id', 'clearance_item']) }}"
                                    >
                                ...
                                @default
                                    <flux:input
                                        ...
                                        label="{{ $cfg['label'] }}"
                                        wire:model.live="formData.{{ $field }}"
                                        required="{{ in_array($field, ['clearance_item']) }}"
                                    />
                                    @error("formData.{$field}")
                                        <flux:text color="red" size="sm">{{ $message }}</flux:text>
                                    @enderror
                ...
                <flux:table.row :key="$item->id">
                            ...
                            <flux:table.cell>
                                @switch($cfg['type'])
                                    @case('switch')
                                        ...
                                        @break
                                    @case('select')
                                        @if($field === 'exit_approval_step_id')
                                            @if($item->exitApprovalStep)
                                                Step {{ $item->exitApprovalStep->flow_order }} - {{ $item->exitApprovalStep->approval_type }}
                                                @if($item->exitApprovalStep->exitEmployeeDepartment)
                                                    ({{ $item->exitApprovalStep->exitEmployeeDepartment->title }})
                                                @endif
                                            @else
                                                N/A
                                            @endif
                    ...
                    <flux:table.cell>
                        ...
                        <flux:modal name="delete-{{ $item->id }}" class="min-w-[22rem]">
                            <div class="space-y-6">
                                <div>
                                    <flux:heading size="lg">Delete Exit Approval Action?</flux:heading>
                                    <flux:text class="mt-2">
                                        <p>You're about to delete this exit approval action. This action cannot be undone.</p>
                                    </flux:text>
                                </div>
