<div class="space-y-6">
    ...
    <div class="flex justify-between">
        ...
        <flux:modal.trigger name="mdl-salary-template-group" class="flex justify-end">
    ...
    <!-- Add/Edit Salary Template Group Modal -->
    <flux:modal name="mdl-salary-template-group" @cancel="resetForm">
        <form wire:submit.prevent="store">
            <div class="space-y-6">
                <div>
                    <flux:heading size="lg">
                        @if($isEditing) Edit Salary Template Group @else Add Salary Template Group @endif
                    </flux:heading>
                    <flux:subheading>
                        @if($isEditing) Update @else Add new @endif salary template group details.
                    </flux:subheading>
                </div>
                ...
                <flux:table.row :key="$item->id">
                            ...
                            <flux:table.cell>
                                @switch($cfg['type'])
                                    @case('switch')
                                        ...
                                        @break
                                    @case('select')
                                        @if($field === 'parent_salary_template_group_id' && $item->$field)
                                            {{ $listsForFields['parent_groups'][$item->$field] ?? $item->$field }}
                                        @elseif($field === 'cycle_unit' && $item->$field)
                                            {{ $listsForFields['cycle_units'][$item->$field] ?? $item->$field }}
                                        @elseif($field === 'cycle_start_unit' && $item->$field)
                                            {{ $listsForFields['cycle_start_units'][$item->$field] ?? $item->$field }}
                                        @else
                                            {{ $item->$field }}
                                        @endif
                                        @break
                    ...
                    <flux:table.cell>
                        ...
                        <flux:modal name="delete-{{ $item->id }}" class="min-w-[22rem]">
                            <div class="space-y-6">
                                <div>
                                    <flux:heading size="lg">Delete Salary Template Group?</flux:heading>
                                    <flux:text class="mt-2">
                                        <p>You're about to delete this salary template group. This action cannot be undone.</p>
                                        <p class="mt-2 text-red-500">Note: Salary template groups with related records cannot be deleted.</p>
                                    </flux:text>
                                </div>
