<div class="space-y-6">
    ...
    <div class="flex justify-between">
        ...
        <flux:modal.trigger name="mdl-payroll-step" class="flex justify-end">
    ...
    <!-- Add/Edit Payroll Step Modal -->
    <flux:modal name="mdl-payroll-step" @cancel="resetForm">
        <form wire:submit.prevent="store">
            <div class="space-y-6">
                <div>
                    <flux:heading size="lg">
                        @if($isEditing) Edit Payroll Step @else Add Payroll Step @endif
                    </flux:heading>
                    <flux:subheading>
                        @if($isEditing) Update @else Add new @endif payroll step details.
                    </flux:subheading>
                </div>
                ...
                <flux:table.row :key="$item->id">
                        ...
                        @if(in_array($field, $visibleFields))
                            <flux:table.cell>
                                @switch($cfg['type'])
                                    ...
                                    @case('select')
                                        @if($field === 'required')
                                            {{ $listsForFields['required_options'][$item->$field] ?? $item->$field }}
                                        @else
                                            {{ $item->$field }}
                                        @endif
                                        @break
                    ...
                    <flux:table.cell>
                        ...
                        <flux:modal name="delete-{{ $item->id }}" class="min-w-[22rem]">
                            <div class="space-y-6">
                                <div>
                                    <flux:heading size="lg">Delete Payroll Step?</flux:heading>
                                    <flux:text class="mt-2">
                                        <p>You're about to delete this payroll step. This action cannot be undone.</p>
                                        <p class="mt-2 text-red-500">Note: Payroll steps with related records cannot be deleted.</p>
                                    </flux:text>
                                </div>
