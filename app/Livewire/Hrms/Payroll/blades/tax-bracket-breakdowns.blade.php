<div class="space-y-6">
    ...
    <div class="flex justify-between">
        ...
        <flux:modal.trigger name="mdl-tax-bracket-breakdown" class="flex justify-end">
    ...
    <!-- Add/Edit Tax Bracket Breakdown Modal -->
    <flux:modal name="mdl-tax-bracket-breakdown" @cancel="resetForm">
        <form wire:submit.prevent="store">
            <div class="space-y-6">
                <div>
                    <flux:heading size="lg">
                        @if($isEditing) Edit Tax Bracket Breakdown @else Add Tax Bracket Breakdown @endif
                    </flux:heading>
                    <flux:subheading>
                        @if($isEditing) Update @else Add new @endif tax bracket breakdown details.
                    </flux:subheading>
                </div>
                ...
                <flux:table.row :key="$item->id">
                    @foreach($fieldConfig as $field => $cfg)
                        @if(in_array($field, $visibleFields))
                            <flux:table.cell class="table-cell-wrap">
                                @switch($cfg['type'])
                                    @case('select')
                                        {{ $listsForFields[$cfg['listKey']][$item->$field] ?? '' }}
                                        @break
                                    @case('switch')
                                        @if($item->$field)
                    ...
                    <flux:table.cell>
                        ...
                        <flux:modal name="delete-{{ $item->id }}" class="min-w-[22rem]">
                            <div class="space-y-6">
                                <div>
                                    <flux:heading size="lg">Delete Tax Bracket Breakdown?</flux:heading>
                                    <flux:text class="mt-2">
                                        <p>You're about to delete this tax bracket breakdown. This action cannot be undone.</p>
                                    </flux:text>
                                </div>
