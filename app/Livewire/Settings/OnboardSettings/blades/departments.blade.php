<div>
    ...
    <div class="flex justify-between">
        ...
        <flux:modal.trigger name="mdl-department" class="flex justify-end">
    ...
    <!-- Modal Start -->
    <flux:modal name="mdl-department" @cancel="resetForm" position="right" class="max-w-none" variant="flyout">
        <form wire:submit.prevent="store">
            <div class="space-y-6">
                <div>
                    <flux:heading size="lg">
                        @if($isEditing) Edit Department @else Add Department @endif
                    </flux:heading>
                    <flux:subheading>
                        @if($isEditing) Update @else Add new @endif department details.
                    </flux:subheading>
                </div>

                <!-- Grid layout for form fields -->
                <div class="grid grid-cols-1 md:grid-cols-1 gap-4">
                    <flux:input label="Title" wire:model="formData.title" placeholder="Department Title"/>
                    ...
                    <flux:select label="Parent Department" variant="listbox" searchable wire:model="formData.parent_department_id"
                               placeholder="Select Parent Department">
                        <flux:select.option value="">-- Select Parent Department --</flux:select.option>
    ...
    <flux:table :paginate="$this->list" class="">
        <flux:table.columns class="bg-zinc-200 dark:bg-zinc-800 border-b dark:border-zinc-700">
            ...
            <flux:table.column>Parent Department</flux:table.column>
        ...
        <flux:table.rows>
            @foreach ($this->list as $rec)
                <flux:table.row :key="$rec->id" class="border-b">
                    <flux:table.cell class="table-cell-wrap">{{ $rec->title }}</flux:table.cell>
                            ...
                            <div class="space-y-6">
                                <div>
                                    ...
                                    <flux:text class="mt-2">
                                        <p>You're about to delete this department. This action cannot be undone.</p>
                                    </flux:text>
                                </div>
