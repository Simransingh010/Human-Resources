<div>
    ...
    <div class="flex justify-between">
        <div class="p-0 m-0">
            <div class="mt-3">
                <flux:heading size="xl" class="mb-1">Modules</flux:heading>
                <flux:text class="mt-2">Manage system modules and their configurations</flux:text>
            </div>
        </div>
        <flux:modal.trigger name="mdl-module" class="flex justify-end">
    ...
    <!-- Modal Start -->
    <flux:modal name="mdl-module" @cancel="resetForm" position="right" class="max-w-none" variant="flyout">
        <form wire:submit.prevent="store">
            <div class="space-y-6">
                <div>
                    <flux:heading size="lg">
                        @if($isEditing) Edit Module @else Add Module @endif
                    </flux:heading>
                    <flux:subheading>
                        @if($isEditing) Update  @else Add new @endif  module details.
                    </flux:subheading>
                </div>

                <!-- Grid layout for form fields -->
                <div class="grid grid-cols-1 md:grid-cols-1 gap-4">
                    <flux:input label="Name" wire:model="formData.name" placeholder="Module Name"/>
                    <flux:input label="Code" wire:model="formData.code" placeholder="Code"/>
                    <flux:input label="Wire" wire:model="formData.wire" placeholder="Wire"/>
                    <flux:input label="Description" wire:model="formData.description" placeholder="Description"/>
                    <flux:input label="Icon" wire:model="formData.icon" placeholder="Icon"/>
                    <flux:input label="Order" wire:model="formData.order" placeholder="Order"/>
                ...
                <flux:table.row :key="$rec->id" class="border-b">
                    ...
                    <flux:table.cell>
                        <div class="flex space-x-2">
                            <flux:button wire:click="showComponentSync({{ $rec->id }})" color="zinc" size="xs">Components</flux:button>
                            <flux:button
                                variant="primary"
                                size="sm"
                                icon="pencil"
                        ...
                        <flux:modal name="delete-{{ $rec->id }}" class="min-w-[22rem]">
                            <div class="space-y-6">
                                <div>
                                    <flux:heading size="lg">Delete Module?</flux:heading>
