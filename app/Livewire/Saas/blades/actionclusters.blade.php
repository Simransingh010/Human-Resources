<div class="space-y-6">
    ...
    <div class="flex justify-between">
        ...
        <flux:modal.trigger name="mdl-actioncluster" class="flex justify-end">
    ...
    <!-- Add/Edit Action Cluster Modal -->
    <flux:modal name="mdl-actioncluster" @cancel="resetForm">
        <form wire:submit.prevent="store">
            <div class="space-y-6">
                <div>
                    <flux:heading size="lg">
                        @if($isEditing) Edit Action Cluster @else Add Action Cluster @endif
                    </flux:heading>
                    <flux:subheading>
                        @if($isEditing) Update @else Add new @endif action cluster details.
                    </flux:subheading>
                </div>
                ...
                <flux:table.row :key="$item->id">
                            ...
                            <flux:table.cell>
                                @switch($cfg['type'])
                                    @case('switch')
                                        ...
                                        @break
                                    @case('select')
                                        @if($field === 'parent_actioncluster_id' && $item->actioncluster)
                                            {{ $item->actioncluster->name }}
                                        @else
                                            {{ $item->$field }}
                                        @endif
                                        @break
                    ...
                    <flux:table.cell>
                        ...
                        <flux:modal name="delete-{{ $item->id }}" class="min-w-[22rem]">
                            <div class="space-y-6">
                                <div>
                                    <flux:heading size="lg">Delete Action Cluster?</flux:heading>
                                    <flux:text class="mt-2">
                                        <p>You're about to delete this action cluster. This action cannot be undone.</p>
                                        <p class="mt-2 text-red-500">Note: Action clusters with related records cannot be deleted.</p>
                                    </flux:text>
                                </div>
