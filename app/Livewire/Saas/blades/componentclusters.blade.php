<div class="space-y-6">
    ...
    <div class="flex justify-between">
        ...
        <flux:modal.trigger name="mdl-componentcluster" class="flex justify-end">
    ...
    <!-- Add/Edit Component Cluster Modal -->
    <flux:modal name="mdl-componentcluster" @cancel="resetForm">
        <form wire:submit.prevent="store">
            <div class="space-y-6">
                <div>
                    <flux:heading size="lg">
                        @if($isEditing) Edit Component Cluster @else Add Component Cluster @endif
                    </flux:heading>
                    <flux:subheading>
                        @if($isEditing) Update @else Add new @endif component cluster details.
                    </flux:subheading>
                </div>
                ...
                <flux:table.row :key="$item->id">
                            ...
                            <flux:table.cell>
                                @switch($cfg['type'])
                                    @case('switch')
                                        ...
                                        @break
                                    @case('select')
                                        @if($field === 'parent_componentcluster_id' && $item->componentcluster)
                                            {{ $item->componentcluster->name }}
                                        @else
                                            {{ $item->$field }}
                                        @endif
                                        @break
                    ...
                    <flux:table.cell>
                        ...
                        <flux:modal name="delete-{{ $item->id }}" class="min-w-[22rem]">
                            <div class="space-y-6">
                                <div>
                                    <flux:heading size="lg">Delete Component Cluster?</flux:heading>
                                    <flux:text class="mt-2">
                                        <p>You're about to delete this component cluster. This action cannot be undone.</p>
                                        <p class="mt-2 text-red-500">Note: Component clusters with related records cannot be deleted.</p>
                                    </flux:text>
                                </div>
