<div class="space-y-6">
    ...
    <div class="flex justify-between">
        ...
        <flux:modal.trigger name="mdl-modulecluster" class="flex justify-end">
    ...
    <!-- Add/Edit Module Cluster Modal -->
    <flux:modal name="mdl-modulecluster" @cancel="resetForm">
        <form wire:submit.prevent="store">
            <div class="space-y-6">
                <div>
                    <flux:heading size="lg">
                        @if($isEditing) Edit Module Cluster @else Add Module Cluster @endif
                    </flux:heading>
                    <flux:subheading>
                        @if($isEditing) Update @else Add new @endif module cluster details.
                    </flux:subheading>
                </div>
                ...
                <flux:table.row :key="$item->id">
                            ...
                            <flux:table.cell>
                                @switch($cfg['type'])
                                    @case('switch')
                                        ...
                                        @break
                                    @case('select')
                                        @if($field === 'parent_modulecluster_id' && $item->modulecluster)
                                            {{ $item->modulecluster->name }}
                                        @else
                                            {{ $item->$field }}
                                        @endif
                                        @break
                    ...
                    <flux:table.cell>
                        ...
                        <flux:modal name="delete-{{ $item->id }}" class="min-w-[22rem]">
                            <div class="space-y-6">
                                <div>
                                    <flux:heading size="lg">Delete Module Cluster?</flux:heading>
                                    <flux:text class="mt-2">
                                        <p>You're about to delete this module cluster. This action cannot be undone.</p>
                                        <p class="mt-2 text-red-500">Note: Module clusters with related records cannot be deleted.</p>
                                    </flux:text>
                                </div>
